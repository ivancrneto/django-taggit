    ...
    def add(self, *tags):
        ...
        for new_tag in str_tags - set(t.name for t in existing):
            try:
                ...
                # We should pass here for case-insensitive tables when the
                # duplication is not with slug
                # (1062, "Duplicate entry 'MyTag' for key 'name'")
                if len(e.args) > 1 and 'duplicate entry' in e.args[1].lower()\
                        and 'slug' not in e.args[1].lower():
                    pass
                else:
                    raise e
